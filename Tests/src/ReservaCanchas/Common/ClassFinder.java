...
public final class ClassFinder {
    ...
    private static List<Class<?>> find(final File file, final String scannedPackage) {
        ...
        } else if (scannedPackage.endsWith(CLASS_SUFFIX)) {
            final int beginIndex = 0;
            final int endIndex = scannedPackage.length() - CLASS_SUFFIX.length();
            final String className = scannedPackage.substring(beginIndex, endIndex);
            try {
